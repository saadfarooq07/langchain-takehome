...
class MemoryService:
            ...
            for memory in memories:
                if memory.value.get("application_name") == application_name:
                    similar_issues.append({
                        ...
                        "documentation_refs": memory.value.get("documentation_refs", []),
                        "log_snippet": memory.value.get("log_snippet", ""),
                        "performance_metrics": memory.value.get("performance_metrics", {})
                    })
    ...
    async def get_application_context(
        ...
        try:
            ...
            memories = await self.store.asearch(
                namespace,
                query=f"application_name:{application_name}",
                limit=1
            )
