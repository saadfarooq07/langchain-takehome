...
def _init_model_sync(config: Optional[RunnableConfig] = None) -> BaseChatModel:
    ...
    if configuration.model.startswith("gemini:"):
        ...
        if "GROQ_API_KEY" not in os.environ:
            raise ValueError("GROQ_API_KEY environment variable is required for Kimi models")
...
def preprocess_log(log_content: str) -> str:
    ...
    for line in lines:
        ...
        # Detect OS/Platform
        for os_key, os_pattern in [(k, v) for k, v in patterns.items() if k.startswith("os_")]:
    ...
    if environment_info["packages"]:
        metadata_section += "\nDetected Packages:\n"
        for pkg, version in list(environment_info["packages"].items())[:10]:  # Limit to first 10
            metadata_section += f"  - {pkg}: {version}\n"
        if len(environment_info["packages"]) > 10:
            metadata_section += f"  ... and {len(environment_info['packages']) - 10} more\n"
